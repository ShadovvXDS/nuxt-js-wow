<template>
    <Popover class="relative" v-slot="{ open }">
        <PopoverButton ref="referenceElement"
            class="group p-[7px] w-11 h-11 justify-center items-center inline-flex rounded-lg ring-1 ring-[#EBF0F3] hover:bg-opacity-5 hover:bg-content-brand"
            :class="{
        'bg-opacity-5 bg-content-brand': open,
    }">
            <svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"
                class="w-5 h-5 group-hover:fill-content-brand" :class="{ 'fill-content-brand': open, }">
                <g opacity="0.5">
                    <path opacity="0.2"
        ...
        " ref="floatingElement" class="absolute z-10 flex flex-col-reverse h-40 w-80" v-slot="{ close }">
                <div class="w-full h-full mb-2 overflow-hidden shadow-lg rounded-xl">
                    <div class="flex flex-col w-full h-full gap-2 p-3 pt-2 bg-white">
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-bold">Negative Prompt</span>
                            <button @click="close">
                        ...
                        <textarea v-model="valueProxy"
                            class="pl-2 pt-4 gap-0.5 rounded-lg w-full h-28 ring-1 ring-border-disabled text-xs resize-none !outline-none focus:ring-2 focus:ring-border-brand-alt"
