<template>
    <section class="overflow-hidden bg-white" id="pricing">
      <div class="max-w-5xl px-8 py-24 mx-auto">
        ...
        <div class="relative flex flex-col items-center justify-center gap-8 lg:flex-row lg:items-stretch">
          <div v-for="(plan, index) in plans" :key="index" class="relative w-full max-w-lg group">
            <div v-if="plan.isFeatured" class="absolute top-0 z-20 -translate-x-1/2 -translate-y-1/2 left-1/2">
              <span class="badge text-xs text-border-brand font-semibold border-1 border-border-brand bg-white">
                POPULAR
              </span>
            </div>
            <div class="absolute -inset-[1px] rounded-[9px] bg-slate-200 z-10"></div>
  
            <div class="relative z-10 flex flex-col h-full gap-5 p-8 rounded-lg lg:gap-8 bg-base-100">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <p class="text-lg font-bold lg:text-xl">{{ plan.name }}</p>
                  <p v-if="plan.description" class="mt-2 text-base-content/80">{{ plan.description }}</p>
                </div>
                <div v-if="plan.freeDuration">
                  <span class="p-1 text-xs font-bold rounded text-content-success bg-surface-success-hover">{{ plan.freeDuration }}</span>
                </div>
              </div>
                ...
                <div class="flex gap-2">
                  <div v-if="plan.priceAnchor" class="flex flex-col justify-end mb-[4px] text-lg">
                    <p class="relative">
                      ...
                      <span class="text-base-content/80">${{ plan.priceAnchor }}</span>
                    </p>
                  </div>
                  <p class="text-5xl tracking-tight font-extrabold group-hover:scale-110 transition-all">${{ plan.price }}</p>
                  <div class="flex flex-col justify-end mb-[4px]">
                    <p class="text-xs font-semibold uppercase text-base-content/60">USD</p>
                  </div>
                </div>
                <div v-if="plan.duration">
                  <span class="px-2 py-2 text-sm rounded bg-surface-brand-hover text-border-brand">{{ plan.duration }}</span>
                </div>
              </div>
              <ul v-if="plan.features" class="space-y-2.5 leading-relaxed text-base flex-1">
                <li v-for="(feature, i) in plan.features" :key="i" class="flex items-center gap-2">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
                    <path d="M17.942 6.34732L7.94205 16.3473C7.884 16.4054 7.81507 16.4515 7.7392 16.483C7.66332 16.5144 7.58199 16.5306 7.49986 16.5306C7.41772 16.5306 7.3364 16.5144 7.26052 16.483C7.18465 16.4515 7.11572 16.4054 7.05767 16.3473L2.68267 11.9723C2.5654 11.855 2.49951 11.696 2.49951 11.5301C2.49951 11.3643 2.5654 11.2052 2.68267 11.0879C2.79995 10.9707 2.95901 10.9048 3.12486 10.9048C3.29071 10.9048 3.44977 10.9707 3.56705 11.0879L7.49986 15.0215L17.0577 5.46294C17.1749 5.34567 17.334 5.27979 17.4999 5.27979C17.6657 5.27979 17.8248 5.34567 17.942 5.46294C18.0593 5.58022 18.1252 5.73928 18.1252 5.90513C18.1252 6.07099 18.0593 6.23005 17.942 6.34732Z" fill="#10B981"></path>
                  </svg>
                  <span>{{ feature.name }}</span>
                </li>
              </ul>
              <div class="space-y-2">
                <ButtonCheckout text="Get NuxtStarter.ai" :withIcon="true" mode="payment" :priceId="plan.priceId" :variantId="plan.variantId" variant="primary" placement="landing_page" :mode="plan.mode"  />
  ...
  export default {
    props: {
      ctaText: {
        type: String,
        default: "Don't waste your time on integrations. Build and ship your app in days."
      }
    },
    components: {
      ButtonCheckout
    ...
    computed: {
      plans() {
        return config.paymentProvider === "stripe" ? config.stripe.plans : config.lemonsqueezy.plans;
      }
    }
  };
  </script>
