<template>
  <section class="flex items-center justify-center w-full overflow-hidden bg-white" id="pricing">
    <div class="flex flex-col items-center justify-center max-w-5xl px-8 py-24 mx-auto">
      <div class="flex flex-col items-center justify-center  w-[80%] mb-20 text-center ">
        <div
          class="px-3 py-2 bg-green-700 bg-opacity-5 rounded-[200px] border border-surface-brand justify-start items-end gap-2 pr-4 flex mb-6">
          ...
          <span class="text-content-brand font-semibold leading-[18.20px] text-sm">
            Pricing
          </span>
        </div>

        <h2 class="text-3xl font-bold tracking-tight lg:text-5xl">
          Don't waste your time on integrations. Build and ship your app in days.
        </h2>

        <div class="text-content-secondary text-[16px] mt-8 flex">
          ...
          <div>
            <span class="inline-flex items-center gap-2 ml-1 font-semibold text-content-brand">
              100$ off
            </span>
            <span>for the first 1000 customers </span>
            <span class="font-semibold">(20 left)</span>
          </div>
        </div>
      </div>

      <div class="relative flex flex-col items-center justify-center w-full gap-8 lg:flex-row lg:items-stretch">

        <div v-for="plan in paymentPlans" :key="(plan as any).priceId ?? (plan as any).variantId" :class="`relative group min-w-0 shrink-0 ${plan.isFeatured ? 'lg:scale-105' : ''
          }`">
          <div v-if="plan.isFeatured" class="absolute top-0 z-20 -translate-x-1/2 -translate-y-1/2 left-1/2">
            <span class="badge text-xs text-white font-semibold border-1 border-border-brand bg-border-brand">
              POPULAR
            </span>
          </div>
          ...
          <div :class="`absolute -inset-[2px] rounded-lg ${plan.isFeatured ? ' bg-border-brand ' : ' bg-border-disabled'
          } z-10`"></div>

          <div class="relative z-10 flex flex-col h-full gap-5 p-8 bg-white rounded-lg lg:gap-8">
            <div class="flex items-center justify-between gap-4">
              <div>
                <p class="text-lg font-bold lg:text-xl">{{ plan.name }}</p>

                <p v-if="plan.description" class="mt-2 text-base-content/80">
                  {{ plan.description }}
                </p>

              </div>

              <div v-if="plan.freeDuration">
                <span class="p-1 text-xs font-bold rounded text-content-success bg-surface-success-hover">
                  {{ plan.freeDuration }}
                </span>
              </div>
            ...
            <div class="flex justify-between">
              <div class="flex gap-2">
                ...
                <div v-if="(plan as any).priceAnchor" class="flex flex-col justify-end mb-[4px] text-lg ">
                  <p class="relative">
                ...
                <p v-if="plan.price"
                  class="text-5xl tracking-tight font-extrabold group-hover:scale-110 transition-all">
                  ${{ plan.price }}
                </p>
                <p v-else class="text-lg font-bold text-base-content/80">
                  Contact us
                </p>



                <div v-if="plan.price" class="flex flex-col justify-end mb-[4px]">
              ...
              <div class="flex xl:block sm:max-xl:max-w-[90px]">
                <span v-if="plan.duration"
                  class="px-2 py-2 text-sm text-center rounded bg-surface-brand-hover text-border-brand">
                  {{ plan.duration }}
                </span>
              </div>
            </div>

            <ul v-if="plan.features" class="space-y-2xs leading-relaxed text-base flex-1 lg:max-xl:max-w-[250px]">

              <li v-for="(feature, i) in plan.features" :key="i" class="flex items-center gap-2">

                <svg v-if="(feature as any).excluded" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                ...
                <span :class="` ${(feature as any).excluded
            ? 'text-base-content/60'
            : 'text-base-content'
          }`">
                  {{ feature.name }}
                </span>
                <span v-if="(feature as any).specialCase"
                  class="text-base-content/60 bg-[#7DFFA2] px-2 text-center rounded-md text-[14px]">
                  {{ (feature as any).specialCase }}
                </span>
              </li>
            ...
            <div class="space-y-2">
              ...
              <NSButton v-if="(plan as any).custom" @click="handleClickContact" size="lg"
                class="w-full mt-8 bg-content-brand">
              ...
              <ButtonCheckout v-else :priceId="(plan as any).priceId" :variantId="(plan as any).variantId"
                placement="landing_page" :mode="(plan.mode as any)" variant="primary" text="Get NuxtStarter AI" />
...
import ButtonCheckout from "@/components/landing-components/ButtonCheckout.vue";
import ButtonLead from "../ButtonLead/ButtonLead.vue";
import NSButton from "@/components/base/NSButton/NSButton.vue";
const paymentPlans = config.paymentProvider === "stripe" ? config.stripe.plans : config.lemonsqueezy.plans

const handleClickContact = () => {
  window.open("mailto:support@mail.nuxtstarter.ai", "_blank");
};
