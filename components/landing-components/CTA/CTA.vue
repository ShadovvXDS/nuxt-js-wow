<template>
  <div class="relative px-8 py-24 mx-auto">
    <div class="absolute top-0 bottom-0 left-0 right-0 z-0 w-full h-full pointer-events-none">
      <img src="/assets/bottom-banner-dots.svg" alt="dots for bottom banner"
        class="absolute object-cover w-full h-full " />
    </div>

    <section class=" rounded-3xl mb-[73px] xl:mb-[100px] mx-auto flex xl:flex-row flex-col " :style="{
      background: 'linear-gradient(135deg, #00C16A 0%, #0C8B52 100%)',
    }">
      <div class="pt-[50px] xl:pt-[68px] pb-[65px] xl:pb-[73px] text-center mx-auto flex flex-col items-center">
        <h3
          class="font-black text-[36px] xs:text-[52px] xl:text-[54px] leading-none font-white mb-[50px] xl:mb-[30px] text-white max-w-[900px] mx-auto">
          {{ description }}
        </h3>

        <div class="mt-8">
          <ButtonCheckout :priceId="config.stripe.plans[1].priceId" :variantId="config.lemonsqueezy.plans[1].variantId"
            placement="landing_page" :mode="config.lemonsqueezy.plans[1].mode" variant="secondary"
            text="Get NuxtStarter" :withIcon="false" />
        </div>
        <span class="flex items-start mt-8">
          ...
          <span class="text-white ">
            <span class="text-[#7DFFA2] ml-1 font-semibold">
              100$ off 
            </span> 
...
const { description = "Don’t overthink, ship your startup in days and start making $$$ online!" } = props;
